      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 relative z-10">
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="space-y-8">
              ...
              {expandedCurso === 'ativo' ? (
                ...
                <motion.div key="collapsed" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="grid lg:grid-cols-2 gap-8">
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-panel rounded-[2.5rem] border border-brand-green/20 overflow-hidden p-8">
                    ...
                    <Button onClick={() => setExpandedCurso('ativo')} className="w-full sm:w-auto h-14 px-8 bg-brand-green hover:bg-brand-neon text-black font-bold rounded-2xl shadow-lg shadow-brand-green/20 transition-all cursor-pointer">
                      Acessar Curso <ChevronRight className="ml-2 w-5 h-5" />
                    </Button>
                  </motion.div>
          ...
          <TabsContent value="atividades" className="space-y-6">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-panel rounded-[2.5rem] border border-white/5 overflow-hidden">
              <div className="p-8 border-b border-white/5 flex items-center justify-between bg-gradient-to-r from-brand-green/10 to-transparent">
                <h3 className="text-2xl font-bold font-display flex items-center gap-3"><FileText className="w-6 h-6 text-brand-neon" /> Minhas Atividades</h3>
                <Badge className="bg-white/5 text-gray-400 border-white/10">{atividades.length} total</Badge>
              </div>
              <div className="p-4 sm:p-8">
                {atividades.length === 0 ? (
                  <div className="text-center py-20"><h4 className="text-xl font-bold text-gray-400">Nenhuma atividade ainda</h4><p className="text-gray-600 mt-2">Suas atividades aparecerão aqui quando forem enviadas pelo professor.</p></div>
                ) : (
                  <div className="grid gap-4">
                    {atividades.sort((a: Activity, b: Activity) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map((atv: Activity) => (
                      <motion.div key={atv.id} whileHover={{ x: 5 }} onClick={() => handleActivityClick(atv)} className="flex flex-col sm:flex-row sm:items-center justify-between p-6 rounded-3xl bg-white/5 border border-white/5 hover:border-brand-green/30 hover:bg-brand-green/5 transition-all cursor-pointer group">
                        <div className="flex items-center gap-4 mb-4 sm:mb-0">
                          <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center group-hover:bg-brand-green/20 transition-colors"><FileText className="w-6 h-6 text-gray-400 group-hover:text-brand-neon" /></div>
                          <div>
                            <h4 className="font-bold text-lg group-hover:text-brand-neon transition-colors">{atv.titulo}</h4>
                            <p className="text-xs text-gray-500 flex items-center gap-1 mt-1"><Clock className="w-3 h-3" /> {new Date(atv.createdAt).toLocaleDateString('pt-BR')}</p>
                          </div>
                        </div>
